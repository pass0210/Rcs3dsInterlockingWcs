...
- **슈트 비움(clear)·PAUSED/RESUMED 운영 조작 인바운드 표면 부재 (2026-07-01 감사 A-확정)**: `OnCleared`/PAUSED 전이를 받을 API·IF가 전무해 슈트가 work_full_qty 도달 시 push ready=false로 사실상 영구 고착(복구는 DB 수기 UPDATE+재시작뿐). ERD의 destination_event(CLEARED/PAUSED/RESUMED) 감사 이력도 생성 경로 없음. **슈트 라인 운영 투입 전 차단 항목** — 관리 API 신설 vs RCS IF 협의 결정 필요. 현 현장(소터 전용)은 무관.
- **RCS 인바운드 API 무인증 + 0.0.0.0:5080 전 인터페이스 바인딩 (2026-07-01 감사)**: 같은 망 임의 호스트가 IF-05 예약·IF-10 투입 보고를 위조 가능(멱등이어도 데이터 오염). 내부망/폐쇄망 전제라면 그 전제를 운영 문서에 명문화하고, 아니라면 최소 인증(API 키 등)·바인딩 제한 결정 필요.
