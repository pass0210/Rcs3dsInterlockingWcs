...
- **C_Flag=1 대기 타임아웃**: R쪽과 달리 상한·알람 미정의(무한 대기 위험). appsettings 설정값 + 초과 시 알람/상태 재확인 — 3DS 협의. **(2026-08-03 S-AUDIT-D-HANDSHAKE-HARDENING D② 반영)**: 상한(`Timing:CFlagTimeoutMs`)·알람(`CFLAG_TIMEOUT`)·상태 매핑(sorter_command/piece = TIMEOUT)은 이미 결선돼 있고, **무한대기 배제**가 결정적 테스트로 회귀 가드됨(C_Flag=1 잔류·미소비 주입 → `Outcome==CFlagTimeout` '단독' 단언 + 경과 ≤ CFlagTimeoutMs+ε: `CFlagTimeoutTests.VS1_*`(오케스트레이터 단위)·`E2EGroupCD.D5b_*`(API/영속화)). **재시도 vs 포기 정책은 여전히 미정**(R_Flag 타임아웃과 동일 — SPEC §7-B) — 현동작(TIMEOUT 저장·재시도 0)은 고정, 3DS 협의 후 결정.
- **IF-10 DENIED piece 재보고 처리 정책 (2026-08-03 S-AUDIT-D-HANDSHAKE-HARDENING D④)**: IF-05에서 NG(DENIED)로 거절된 piece에 RCS가 IF-10(투입 보고)을 보내는 경우의 처리. **현동작 확정·고정**: 200 OK 멱등 응답 유지 + 투입 차단(piece DENIED 불변·piece_event 무증가) + `_log` WARN·operation_log(`IF10_DENIED_REREPORT`, WARN) 관측 기록. **정책 전환(예: DENIED 재보고를 alarm으로 승격, RCS에 NG 회신 등)은 이번 미포함** — RCS 협의 후 결정. 마찬가지로 **미존재 chuteNo·무피스 IF-10**(활성 piece 없고 chuteNo 목적지도 없음)도 200 + WARN(`IF10_NO_DESTINATION`)·기록 불가로 고정. 원인 구분은 `IDepositRecorder.RecordDeposit`이 `DepositRecordResult`(NewRecord/Duplicate/DeniedReport/NoDestination)로 반환(구 bool false 3원인 '멱등 OK' 합류 해소).
- **sorter_command SENT 행 내구화 시점 — 수용된 갭 (2026-08-03 S-AUDIT-D-HANDSHAKE-HARDENING D③-part2 SCOPE OUT)**: `EfSorterCommandJournal.CreateSent`(sorter_command SENT 행 생성)는 현재 **핸드셰이크 완료 후** 컨트롤러 백그라운드 continuation에서 `Finalize`와 함께 호출된다(투입 직후 즉시 내구화 아님). C 기입 직후~핸드셰이크 완료 사이에 프로세스가 크래시하면 그 건의 sorter_command 행이 없을 수 있으나, **크래시 감사 앵커는 `HS_C_SENT` operation_log가 이미 커버**(C 투입 시점 구조화 기록)하므로 수용된 갭으로 둔다. 투입 직후 SENT 행 선-내구화(2단계 저널)는 후속 스프린트 검토 — 저널 API 재설계·2-write 트랜잭션 긴장이 있어 저우선.
...
- **IF-05 동일 바코드 다중 목적지 선택 규칙 → ✅ 확정(2026-07-27, S-B2C-BARCODE-MULTI-FIX Fix 2)**: `order_item` UQ가 `(order_id, barcode)`라 **같은 바코드가 여러 활성 오더(다른 destination)에 존재 가능**(교차-배치 중복 업로드로 현장 실발생). 구 `DbRepositories.QueryDestination`은 `.Where(barcode 일치 && Order.Status∉{COMPLETED,CANCELLED}).FirstOrDefault()`로 **정렬 기준 없이 첫 매치** 반환 → 다중 매치 시 비결정적으로 미배정 오더를 골라 NG/NO_DEST 를 뱉었다. **확정 규칙 = 배정-우선 결정적 선택**(순수 함수 `Wcs.Core.BarcodeDestinationSelector`, 절대규칙 #8): (1) 후보 중 `order.DestinationId != null`(배정)이 있으면 그 그룹 우선, (2) tiebreak = 배정확정 최신 `DestAssignedAt`(내림차순·null=MinValue) → 최소 `OrderId` → 최소 `OrderItemId`(PK 전순서 잠금·완전 결정적), (3) 배정 후보 없으면 미배정에서 같은 정렬로 결정적 1건 → 이후 기존 AUTO 슈트배정/NO_DEST 흐름 불변. 단건 1:1 매치는 그 후보 반환(회귀 0). 선택 이후 상태판정·예약차감·piece 삽입·트랜잭션·RecordDenied 경로 전부 불변. 테스트: 순수 규칙 단위(`BarcodeDestinationSelectorTests`) + IF-05 다중오더 배정우선·tiebreak 통합(`B2cBarcodeMultiFixTests`).
  - **경로 일관성(조사 결론)**: "바코드→목적지" 모호성은 **IF-05 `QueryDestination` 고유**다. 다른 바코드-해소 경로는 모호성이 없다 — IF-09 `EfArrivalRecorder.RecordArrival`·IF-10 `EfDepositRecorder.RecordDeposit`는 활성 piece 를 **PId 로 조회**(바코드 아님·piece 는 IF-05 에서 이미 목적지 확정됨), IF-10 신규-pId fallback 은 `chuteNo`로 destination 직조회(바코드→오더 아님). `EfCellSelector.SelectCell`·`ReleaseEmptyAssignment`는 바코드로 조회하나 이미 **destination(chuteNo) 스코프**라 교차-목적지 모호성이 구조적으로 없다(SelectCell②의 배정 오더 조회도 `Order.DestinationId == dest.Id` 한정). 따라서 동일 결정적 규칙을 다른 경로에 적용할 필요 없음.
