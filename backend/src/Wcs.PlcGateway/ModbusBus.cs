...
public sealed class ModbusBus : IAsyncDisposable
{
    ...
    /// <summary>
    /// 이 버스에 unitId 슬레이브를 추가한다. per-slave 어댑터(BusSlaveMaster)로 공유 연결에 결선한
    /// PlcPollingService를 생성해 반환(HandshakeOrchestrator는 이 반환값을 그대로 감싼다).
    /// </summary>
    public PlcPollingService AddSlave(byte unitId, ILogger<PlcPollingService>? log = null)
    {
        ...
        var svc = new PlcPollingService(_opt, new PlcWriteQueue(), master, log);
