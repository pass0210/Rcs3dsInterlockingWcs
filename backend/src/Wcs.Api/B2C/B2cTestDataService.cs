...
//   COMPLETED 오더는 RUNNING 으로 재개(재테스트 가능 — QueryDestination 이 COMPLETED 제외하므로
//   재개하지 않으면 같은 바코드 재투입이 NG). in-flight piece 존재 시 기본 거부·force 로만 허용(OQ3).
//
...
public interface IB2cTestDataService
{
    ...
    /// <summary>초기화(재테스트 준비) — 아카이브 + 수량 리셋 + 오더 재개. in-flight 가드(force).</summary>
...
public sealed class B2cTestDataService : IB2cTestDataService
{
    ...
    {
        var dest = await _db.Destinations
            .FirstOrDefaultAsync(d => d.ChuteNo == req.SorterChuteNo && d.DestType == DestType.SORTER_3D, ct)
            .ConfigureAwait(false);
        if (dest is null)
        {
            var msg = $"소터 chuteNo={req.SorterChuteNo} 를 찾을 수 없습니다(SORTER_3D 아님/미존재).";
            Audit("B2C_RESET", OperationLogLevel.WARN, req.SorterChuteNo, null, msg);
        ...
        try
        {
            ...
            //   채널 enqueue 라 트랜잭션과 무관).
            int inFlight = await _db.Pieces
                .Where(p => p.DestinationId == dest.Id)
                .Where(IsInFlight)
                .CountAsync(ct).ConfigureAwait(false);
            ...
            if (inFlight > 0 && !req.Force)
            {
                ...
                Audit("B2C_RESET", OperationLogLevel.WARN, req.SorterChuteNo, dest.Id,
                    $"{{\"refused\":true,\"inFlight\":{inFlight},\"force\":false}}");
                return B2cManagementResponse.Fail(msg, new Dictionary<string, int> { ["inFlight"] = inFlight });
            }

            // 대상 piece id(아직 활성=archived_at NULL)를 먼저 캡처 — 연관 이벤트/명령 아카이브에 사용.
            var pieceIds = await _db.Pieces
                .Where(p => p.DestinationId == dest.Id && p.ArchivedAt == null)
                .Select(p => p.Id)
                .ToListAsync(ct).ConfigureAwait(false);
            ...
            // order_item reserved/sorted = 0 리셋(소터 소속 오더) — wcs_order/cell_assignment 는 보존(OQ2).
            var orderIds = await _db.Orders
                .Where(o => o.DestinationId == dest.Id)
                .Select(o => o.Id)
                .ToListAsync(ct).ConfigureAwait(false);
            ...
            var completedOrders = await _db.Orders
                .Where(o => o.DestinationId == dest.Id && o.Status == OrderStatus.COMPLETED)
                .ToListAsync(ct).ConfigureAwait(false);
            ...
            var message = $"초기화 완료 — 소터 {req.SorterChuteNo}: 피스 {archivedPieces}건 보관, "
                        + $"이력 {archivedEvents}·명령 {archivedCommands}건 보관, 수량 리셋 {resetItems}건, 오더 재개 {reopened}건"
                        + (req.Force && inFlight > 0 ? $" (강제 — 진행 중 {inFlight}건 포함)" : "");
            Audit("B2C_RESET", OperationLogLevel.INFO, req.SorterChuteNo, dest.Id,
                $"{{\"archivedPieces\":{archivedPieces},\"archivedEvents\":{archivedEvents},\"archivedCommands\":{archivedCommands},\"resetItems\":{resetItems},\"reopened\":{reopened},\"force\":{(req.Force ? "true" : "false")}}}");
            return B2cManagementResponse.Ok(message, counts);
        }
        catch
        {
